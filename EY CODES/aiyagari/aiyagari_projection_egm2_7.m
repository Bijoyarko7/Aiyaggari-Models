function [f1,aggk,aggn,v2,optk2,optc2] = aiyagari_projection_egm2_7(r,alpha,beta,delta,zgrid,piz,kgrid,f1,v2,optk2,optc2,hot)

w = (1-alpha)*(r/alpha)^(alpha/(alpha-1));

if hot == 0
    v2 = zeros(51,7); optk2 = zeros(151,7); optc2 = zeros(151,7);
    for i=1:7
...
for iter = 1:9999
    ...
    for j = 1:7
        for k = 1:7
            ev(:,j)=ev(:,j)+piz(j,k)*v(:,k);
        end
        pvv = pchip(kgrid,ev(:,j));
